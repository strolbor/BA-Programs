...
df1 = pd.read_csv("03-Forklift-kconfigreader-median.csv")
df2 = pd.read_csv("03-Forklift-kmax-median.csv")
...
def plotter():
    ...
    plt.plot(df1['Year-DIMACS'], df1['dimacs-analyzer-time'], marker='o')

    plt.scatter(df2['Year-DIMACS'], df2['dimacs-analyzer-time'], label='KMAX', color='red')
    plt.plot(df2['Year-DIMACS'], df2['dimacs-analyzer-time'], marker='o')
    ...
    plt.grid(True)
    plt.savefig(os.path.join(f'forklift.png'), bbox_inches='tight')
    plt.savefig(os.path.join(f'forklift.svg'), bbox_inches='tight')
