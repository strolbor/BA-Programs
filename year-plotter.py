...
def plotter_fkt(data_dict, dateiname,title):
    ...
    for solver, data in data_dict.items():
        versions = []
        times = []
        satisfiable = []  # Liste für model-satisfiable
        for row in data:
            versions.append(row['dimacs-analyzer'].split('/')[-1].split('[')[0])
            times.append(int(row['dimacs-analyzer-time']))
            satisfiable.append(row['model-satisfiable'])  # Daten für model-satisfiable hinzufügen
        ...
        # Wende die gleiche Reihenfolge auf die Zeiten und model-satisfiable Daten an
        times_sorted = [times[versions.index(version)] for version in versions_sorted]
        satisfiable_sorted = [satisfiable[versions.index(version)] for version in versions_sorted]
        ...
        # Markiere die Punkte, deren model-satisfiable nicht 'true' ist
        for i, sat in enumerate(satisfiable_sorted):
            if sat != 'true':
                plt.scatter(versions_sorted[i], times_sorted[i], color='red', marker='x')
